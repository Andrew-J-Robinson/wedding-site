...
module.exports = async function handler(req, res) {
  ...
  const updates = order.map((id, idx) =>
    supabase.from('checklist').update({ sort_order: idx }).eq('id', id)
  );
  await Promise.all(updates);

  const { data } = await supabase.from('checklist').select('*').order('sort_order');
  const mapped = (data || []).map((t) => ({ id: t.id, title: t.title, order: t.sort_order, completed: t.completed, createdAt: t.created_at }));
  res.json(mapped);
};
